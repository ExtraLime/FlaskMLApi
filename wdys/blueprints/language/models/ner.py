...
nlp = pipeline(task='token-classification',tokenizer=full_path, model=full_path)
    ...
    def get_ents(self, input_string:str=''):
        ...
        for i in range(len(raw_result)):
            if i>0 and raw_result[i]['word'].startswith('##'):
                entities[-1] = {raw_result[i-1]['word']+raw_result[i]['word'].replace('##','') : raw_result[i]['entity']}
            else:
                entities.append({raw_result[i]['word']: raw_result[i]['entity']})    
